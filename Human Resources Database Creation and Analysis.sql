...
-- Which locations have the highest and lowest average gross salary?
SELECT 
    l.CountryName, 
    ROUND(AVG(s.GrossAmount),0) AS AvgGrossSalary
FROM Employee AS e
INNER JOIN Salary AS s ON e.ID = s.EmployeeID
INNER JOIN [Location] AS l ON e.LocationID = l.ID
...
SELECT
    ...
    d.[Name],
    s.GrossAmount,
    RANK() OVER (PARTITION BY d.[Name] ORDER BY s.GrossAmount DESC) AS SalaryRank
FROM
    Employee AS e
INNER JOIN
    Department AS d ON e.DepartmentID = d.ID
INNER JOIN
    Salary AS s ON e.ID = s.EmployeeID
	)
SELECT FirstName, LastName, [Name] AS DepartmentName, GrossAmount
FROM CTE
WHERE SalaryRank = 1;